...
class SimpleFormFillSignature(dspy.Signature):
    """
    Extract medical terms for: {answer_field_name}
    
    Description: {answer_field_description}
    
    STRICT RULES:
    1. Extract EXACTLY 1-3 terms (NEVER more than 3)
    2. If >3 relevant terms exist, select ONLY the 3 MOST relevant
    3. Terms must be explicitly stated in the context
    4. Use complete medical terminology
    5. Separate multiple terms with commas
    6. If no relevant terms exist, output "Not specified"
    
    SELECTION CRITERIA when >3 terms available:
    - Most specific to the field question
    - Most clinically significant
    - Most frequently mentioned in context
    
    EXAMPLES:
    - Disease field → {{"answer": "melanoma, BRAF mutation, skin cancer"}}  [MAX 3]
    - Treatment field → {{"answer": "pembrolizumab, chemotherapy"}}  [2 terms only]
    - Single finding → {{"answer": "Type 2 diabetes mellitus"}}  [1 term only]
    - Nothing relevant → {{"answer": "Not specified"}}
    
    IMPORTANT: Maximum 3 terms. Choose the most relevant if more exist.
    """
...
class CoTFormFillSignature(dspy.Signature):
    """
    You are an expert biomedical information extractor analyzing scientific literature.

    CONTEXT
    =======
    {context}

    TASK: Extract information for field "{answer_field_name}"
    DESCRIPTION: {answer_field_description}

    OUTPUT SCHEMA
    -------------
    {{
    "reasoning": "<Explain your analysis: What relevant terms did you find? Why did you select these specific ones over others? How do they best answer the field question?>",
    "evidence": {{
        "<term1>": "<verbatim quote containing term1>",
        "<term2>": "<verbatim quote containing term2>",  
        "<term3>": "<verbatim quote containing term3>"
    }},
    "final_answer": ["<term1>", "<term2>", "<term3>"]
    }}

    EXTRACTION RULES
    ----------------
    1. Scan entire context for ALL potentially relevant terms
    2. Select the 1-3 MOST relevant terms (NEVER more than 3)
    3. Prioritize by:
       - Direct textual evidence in context
       - Specificity to the field question  
       - Clinical/scientific significance
    4. Evidence MUST be exact quotes containing the term
    5. Empty results format:
       {{
       "reasoning": "Searched for [specific terms related to field] but found no direct evidence in the provided context.",
       "evidence": {{}},
       "final_answer": []
       }}
    ...
    ----------------------
    - disease_name: The PRIMARY disease/condition studied (usually in title/abstract)
    - etiology_factor: Direct causes, mutations, or triggers explicitly mentioned
    - diagnostic_method: Specific tests, procedures, or criteria used for diagnosis
    - biomarker: Measurable indicators of disease state or progression
    - treatment_intervention: Active therapies, drugs, or procedures (not supportive care)
    - prognostic_indicator: Factors that predict outcomes or survival
...
# Simple schema for JSON mode without CoT
class SimpleAnswerSchema(pydantic.BaseModel):
    answer: str
...
class FieldFiller(dspy.Module):
    ...
    """
    def __init__(self, answer_generator, signature, answer_in_quotes=False, listify = False, verbose=False, use_cot=False):
    ...
    def forward(self, prompt_input, context, field_type):
        ...
        if self.verbose:
            ...
            if hasattr(self, 'use_cot'):
                print("              Using CoT: ", self.use_cot)
        ...
        if hasattr(self, 'use_cot') and self.use_cot:
            # CoT mode with retry logic and JSON parsing
            max_retries = 2
            for attempt in range(max_retries):
                try: 
                    raw_llm_output = self.answer_generator(self.predictor, **prompt_input)
                    
                    # Quick JSON repair for common truncation issues
                    if not raw_llm_output.rstrip().endswith('}'):
                        # Try to close truncated JSON
                        raw_llm_output = raw_llm_output.rstrip()
                        if raw_llm_output.count('{') > raw_llm_output.count('}'):
                            raw_llm_output += '"}' * (raw_llm_output.count('{') - raw_llm_output.count('}'))
                    
                    # Import here to avoid circular dependency
                    from ..metadata_schemas.cot_schema import CoTModelSchema
                    parsed_data = CoTModelSchema.model_validate_json(raw_llm_output)
                    ...
                    if self.verbose:
                        print(f"    --CoT--: Raw LLM Output: {raw_llm_output}")
                        print(f"    --CoT--: Reasoning: {parsed_data.reasoning}")
                        print(f"    --CoT--: Evidence: {parsed_data.evidence}")
                        print(f"    --CoT--: Final Answer: {parsed_data.final_answer}")

                    # Return the list directly (no join) or "Not specified"
                    return parsed_data.final_answer if parsed_data.final_answer else ["Not specified"]
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f"WARNING: Failed to parse CoT JSON after {max_retries} attempts")
                        print(f"Error: {e}")
                        return ["Not specified"]  # Return list for consistency
        
        elif hasattr(self, 'signature') and self.signature == SimpleFormFillSignature:
            # Simple JSON mode (no CoT)
            raw_llm_output = self.answer_generator(self.predictor, **prompt_input)
            
            if self.verbose:
                print(f"    --INFO--: Raw LLM Output (JSON): {raw_llm_output}")
            ...
            try:
                ...
                print(f"WARNING: Failed to parse simple JSON: {raw_llm_output}")
                print(f"Error: {e}")
                return "Not specified"
        
        else:
            # Original mode (regex constraints)
            ...
            if self.listify:
                ...
                answers = [parse_single_output(field_type, answer) for answer in answers]
                return answers
...
class SequentialFormFiller(dspy.Module):
    ...
    def __init__(self,
        ...
        self.cot_schema = cot_schema
        
        # Signature selection logic
        if use_cot and cot_schema:
            self.signature = CoTFormFillSignature
            self.max_tokens = max(max_tokens, 2500)  # Increase for CoT
        elif use_json_constraints and not use_cot:
            self.signature = SimpleFormFillSignature
            self.max_tokens = max(max_tokens, 200)   # Increase for JSON
        elif listify_form:
            self.signature = ListedFormFillSignature
        else:
            self.signature = FormFillSignature
    ...
    def set_pydantic_form(self, pydantic_form):
        ...
        if self.verbose:
            mode_desc = "CoT JSON" if self.use_cot else ("Simple JSON" if self.use_json_constraints else "Regex")
            print(f"Generating {mode_desc} generators...")

        # iterate through fields
        fields = pydantic_form.model_fields
        for fieldname in fields:
            field_type, min_l, max_l = get_constraints_from_field(fields[fieldname])

            # Create generator key that accounts for mode
            generator_key = (field_type, min_l, max_l, self.use_cot, self.use_json_constraints, self.listify_form, self.answer_in_quotes)

            # only make a new generator if it is not equal to one already generated
            if not generator_key in self.dspy_generators:
                
                if self.use_json_constraints or self.use_cot:
                    # JSON mode
                    if self.use_cot and self.cot_schema:
                        schema_for_generator = self.cot_schema
                    else:
                        schema_for_generator = SimpleAnswerSchema
                    ...
                    outlines_generator = make_constrained_generator(
                            ...
                            pydantic_schema=schema_for_generator)
                    
                    generator_max_tokens = 2500 if self.use_cot else 200
                else:
                    # Original regex mode
                    outlines_generator = make_constrained_generator(
                            llm_model=self.llm_model,
                ...
                self.dspy_generators[generator_key] = make_dspy_generator(self.llm_model, outlines_generator, max_tokens = generator_max_tokens)
        if self.verbose:
            print("Finished generating generators.")

        self.prepare_field_fillers()

    def prepare_field_fillers(self):
        self.field_fillers = {}
        fields = self.pydantic_form.model_fields
        for fieldname in fields:
            field = fields[fieldname]
            field_type, min_l, max_l = get_constraints_from_field(field)
            generator_key = (field_type, min_l, max_l, self.use_cot, self.use_json_constraints, self.listify_form, self.answer_in_quotes)
            generator = self.dspy_generators[generator_key]
    ...
    @weave.op()
    def forward(self, get_context, exclude_fields = [], progress_callback=None):
        ...
        # Evidence tracking for CoT mode
        if self.use_cot:
            self.evidence = {}
        ...
        for i, fieldname in enumerate(fields):
            ...
            # Handle evidence tracking for CoT mode
            if self.use_cot:
                ff = self.field_fillers[fieldname]
                if hasattr(ff, "last_cot_data") and ff.last_cot_data is not None:
                    self.evidence[fieldname] = ff.last_cot_data.get("evidence", {})
                
                # CoT returns lists, convert to comma-separated string for pydantic
                if isinstance(output, list):
                    cleaned_output = ", ".join(output) if output and output != ["Not specified"] else "Not specified"
                else:
                    cleaned_output = output
            else:
                cleaned_output = output

            output_dict[fieldname] = cleaned_output

            # Progress callback for UI
            if progress_callback:
                current_progress_value = (i + 1) / len(fields)
                progress_text = f"Processed field {i+1}/{len(fields)}: {fieldname}"
                try:
                    progress_callback(current_progress_value, text=progress_text)
                except Exception as e:
                    if self.verbose:
                        print(f"Error calling progress_callback: {e}")
    ...
    for field in original_fields:
        if not field in exclude_fields:
            ...
            if remove_maxlength_and_examples:
                ...
                new_fields[field] = (original_schema.model_fields[field].annotation, pydantic.Field(**properties))
            else:
...
class OpenAISequentialFormFiller(dspy.Module):
    ...
    def forward(self, get_context, exclude_fields = []):
        ...
        for fieldname in fields:
            ...
            all_other_fields = list(self.pydantic_form.model_fields.keys())
