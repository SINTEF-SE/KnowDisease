...
class OutlinesHFModel(LM):
    ...
    def _generate(self, prompt, **kwargs):
        ...
        # Convert Pydantic model to JSON string if needed (for JSON schema generation)
        if hasattr(outputs, 'model_dump_json'):
            # It's a Pydantic model, convert to JSON string
            outputs = outputs.model_dump_json()
        elif hasattr(outputs, 'json'):
...
def make_constrained_generator(field_type, llm_model, min_l, max_l, answer_in_quotes, listify_form, sampler = None, pydantic_schema = None):
    """
    ...
    * the output is always a string, but a parsable one
    ...
    if pydantic_schema:
        # Use JSON schema generation for structured output
        generator = outlines.generate.json(llm_model, pydantic_schema, sampler=sampler)
    else:
        # Use original regex-based generation
